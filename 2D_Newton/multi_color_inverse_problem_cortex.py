...
image_int = io.imread('cortex.png')
...
'''
Image.fromarray(image_int[:, :, 0]).show()
Image.fromarray(image_int[:, :, 1]).show()
Image.fromarray(image_int[:, :, 2]).show()
Image.fromarray(image_int[:, :, 3]).show()
...
A = np.random.rand(int(0.75 * img_size[1]), img_size[0])
...
#Initialization
rec_bromo = np.random.rand(img_size[0], img_size[1])

x_rgb_prog_list = [[], [], []]

for color in range(3):
    x_color_0 = np.random.rand(img_size[0], img_size[1])
...
for i in range(10):
    ...
    rec_img = Image.fromarray(reconstructed_img, 'RGB')
    rec_img.save('rec_cortex_%s.png' % i)
